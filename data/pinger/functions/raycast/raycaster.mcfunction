...
# エンティティ検出時にdetect_entity実行（ノーマルジャンプ時のみ有効）
...
# ブロック検出時にdetect_block実行（スニークジャンプ時のみ有効）
...
# # 何も検出されなかったときにdetect_outrange実行
# execute if score @s PINGER_FLAG_raycast_detection matches 0 if score @s PINGER_raycast_count matches 1000 run function pinger:raycast/detect_outrange
